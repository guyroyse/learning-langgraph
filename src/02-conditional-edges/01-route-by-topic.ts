...
import { zorkRouter, zorkRejector, zorkAgent, zorkFactChecker } from './agents/index.js'

// -----------------------------------------------------------------------------
// Pattern 1: Route by Topic (router decides based on LLM classification)
//
// START ──→ zork_router ──┬──→ zork_agent ──→ zork_fact_checker ──→ END
//                         ╰──→ zork_rejector ──→ END
// -----------------------------------------------------------------------------
...
export async function runRouteByTopic(question: string) {
  console.log(chalk.gray('Pattern: Route by Topic (LLM classifies → conditional edge)'))
  console.log()
